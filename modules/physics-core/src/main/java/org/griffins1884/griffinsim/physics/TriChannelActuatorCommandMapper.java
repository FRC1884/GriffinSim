...
public final class TriChannelActuatorCommandMapper implements ActuatorCommandMapper {
  ...
  public TriChannelActuatorCommandMapper(
    ...
    if (xChannel < 0 || yChannel < 0 || thetaChannel < 0) {
    ...
    this.thetaChannel = thetaChannel;
    this.massKg = massKg;
  ...
  public List<BodyCommand> map(ActuatorFrame frame, ImmutableWorldState currentWorld) {
    ...
    for (PwmOutput pwmOutput : frame.pwmOutputs()) {
      ...
      } else if (pwmOutput.channel() == thetaChannel) {
        torqueZ += pwmOutput.value() * torqueScale;
      }
    }
